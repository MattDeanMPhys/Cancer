...
test2 = log(unlist(posPop[nrow(posPop)*t,]/posPop[1,1]))
Frame2 = data.frame(c(1:ncol(posPop)), test2, rep.int("t=2", ncol(posPop)))
names(Frame2) = c("x", "Population", "Time")

linsec = log(unlist(posPop[nrow(posPop)*t,(linstart):(linend)])/posPop[1,1])
xt = seq(linstart,linend,length.out=(length(linsec)))
res=lm(linsec~xt)

dFrame = rbind(Frame2)

graph = ggplot(dFrame, aes(x, Population, group=Time))  + geom_point() + theme_bw() + theme(panel.border = element_rect(colour = "black"), panel.grid.major = element_blank(), panel.grid.minor = element_blank())
graph = graph + theme(legend.position = "none") + scale_x_continuous("Mutation Number", expand=c(0,0), limits=c(linstart,linend)) + scale_y_continuous("Log Concentration", expand=c(0,0))
...
t=0.4 #as fraction of total time

test2 = log(unlist(flatPop[nrow(flatPop)*t,]/flatPop[1,1]))
Frame2 = data.frame(c(1:ncol(flatPop)), test2, rep.int("t=2", ncol(flatPop)))
names(Frame2) = c("x", "Population", "Time")

linstart = 64
linend = 76

linsec = log(unlist(flatPop[nrow(flatPop)*t,(linstart):(linend)])/flatPop[1,1])
xt = seq(linstart,linend,length.out=(length(linsec)))
res=lm(linsec~xt)
...
graph = graph + geom_abline(intercept = res[[1]][[1]], slope = res[[1]][[2]], aes(color='red'))
tikz(file = "exp_simdata_flat.tex", width = 3, height = 3)
print(graph)
dev.off()
