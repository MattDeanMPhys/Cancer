...
std::cout << "Parameters Written and Populations Created" << std::endl;
...
if( uLandscape.compare("FILE") == 0){
	...
	std::ifstream inputFile (uFileName);

	if (inputFile.is_open()){

		while( getline(inputFile, line)) { 
	
			uFileInput.push_back(std::stod(line)) ;
		}

		inputFile.close();
	}
	
	else std::cout << "Unable to open Mutation File" << std::endl ;

	pop_test.Manual_MutationLandscape(uFileInput);
...
std::cout << "Landscapes Built" << std::endl;
